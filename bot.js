...
client.on("guildMemberAdd", (member, message) => {
  if (member.guild.id !== "465827492872323072") return; //sunucu ıd
  let aylartoplam = {
    "01": "Ocak",
    "02": "Şubat",
    "03": "Mart",
  ...
  const id = "760986687094259712"; //kanal ıd
 ...
 var kontrol;
  ...
  channel.send(
    ...
    member.guild.members.size
      ...
      "YYYY HH:mm:ss"
    )}  \n  **Bu Hesap ${kontrol}**\n <@&753626933308293230> Rolündeki yetkililer sizinle ilgilenecektir.`
  , {files: [`https://cdn.discordapp.com/attachments/681261346393948222/759760194258731039/vcvc.gif`]});
});
//--------OTOROL------//
client.on('guildMemberAdd', member  => {
  let rol = "758672793532497921"//VERİLMESİNİ İSTEDİĞİNİZ ROLÜN İDSİ
  client.channels.get("759823090661851222").send(`${member} Adlı Kullanıya Başarıyla Otorol Verıldı`)
  member.addRole(rol)
})
//-----------ototag-------//
client.on("userUpdate", async (old, nev) => {
  let emingSunucu = "465827492872323072"; //Sunucu 760995291980627988ID
  let emingKanal = "760995291980627988"; //BILGI KANAL ID
  let emingRol = "750436762253262918"; //ROL ID
  let emingTag = '℘'; //TAG
...
////hoşgeldin-dm-mesaj////
client.on('guildMemberAdd', member => {
 member.send(`**℘ ASWENG FAMILY ℘

Merhaba! 1 dakikanı ayırabilir misin?

 ℘ Sunucumuzda yarattığımız sıcak ve samimi arkadaş ortamımıza sende katılıp yeni arkadaşlar edinebilirsin.

 ℘ Kendine yeni oyun arkadaşları bulabilir ve hatta yeni oyunlar keşfedip oynamaya başlayabilirsiniz.

 ℘ Düzenli yaptığımız sunucu içi oyunlar (vampir-köylü vb.) ve çekilişlere katılabilir ödüller kazanabilirsiniz.

 ℘ Tag alarak ve sunucumuza boots basarak sunucumuzun daha farklı ayrıcalıklarından faydalanabilir hatta yetkili olabilirsiniz.

Ailemize katılmak için davet linki: https://discord.gg/dpyB9FD


℘ 𝔸𝕊𝕎𝔼ℕ𝔾 ℘**`);//istediğiniz mesaj
});  
////////emoji-sa-as//////
...
//-----sesteafkbırakma---//
client.on('ready', ()=>{
client.channels.get('761151972370939934').join() //SESKANILIN İD Sİ
})
//-----botun yazdığı kanal----//
client.on("ready", async () => {
  var channel = client.channels.get("760986687094259712"); // YAZIYOR GÖRÜNMESİNİ İSTEDİĞİNİZ KANAL İD
  function Lewis(kod) {
   kod.startTyping();
  }
 Lewis(channel);
